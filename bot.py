...
- 最终判定自检：是不是三选一？有没有"不太适合""可能需要""或者调整"等模糊词？判定依据写清楚了吗？
- 创始人评级自检：有没有给身价量级区间？上下限依据是什么？有没有标注"命理娱乐参考"？
...
5. 【最终判定】锁死规则：
   - 必须三选一：天命所归 / 需要调整 / 逆天而行。不允许第四种结论。
   - 禁止使用"不太适合""可能需要""或者调整""不太舒服"等模糊表述。
   - 必须写清楚：为什么是这个判定？核心依据是什么？如果是逆天而行，具体怎么死？如果是天命所归，具体怎么赢？
   - 2-3段论证，不是扔标签。

6. 【创始人评级】规则：
   - 基于三个维度交叉判断：①格局层级（七杀配印/伤官生财/财官双美等决定财富上限）②行业天花板（奶茶加盟≤千万级，消费品牌≤十亿级，科技/半导体≤百亿级）③大运窗口（财富运在哪个十年，是早年爆发还是中年起势）。
   - 给出身价区间，必须写明上限和下限的依据。例："格局伤官生财有上限，行业消费品牌天花板约十亿级，大运34岁后忌神运收缩，预估身价区间：1-5亿（上限受行业天花板约束，下限受大运收缩影响）。"
   - 不同命格不同行业差异巨大：干奶茶的可能千万封顶，做半导体的可能百亿波动，必须具体问题具体分析，不能给统一答案。
   - 必须标注："命理娱乐参考，不构成投资建议。"
   - 不要给精确数字，给量级区间（百万/千万/亿/十亿/百亿）。

7. 不要在报告结尾加"追问钩子""追问1/2/3"。报告写完即止。
"""
        max_tokens = 16384
    else:
        tier_instructions = """
...
4. 【最终判定】必须三选一（天命所归/需要调整/逆天而行），禁止"不太适合""可能需要"等模糊词。
5. 【创始人评级】一句话给身价量级区间（百万/千万/亿/十亿/百亿），基于格局+行业天花板+大运，标注"命理娱乐参考"。
6. 报告结尾加一行：
